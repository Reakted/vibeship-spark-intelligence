...
## Benchmark-Answer Alignment (Make Questions Measurable)

Goal:
- Ensure each question maps to a measurable benchmark signal.
- If a question cannot be answered by current metrics, it becomes a required
  instrumentation gap.

Answer protocol (apply per question):
1) Identify the primary metric(s).
2) Identify the scenario(s) and logs used.
3) Record evidence from `benchmarks/out/*/report.json`.
4) If no metric exists, add a "missing metric" note.

Minimum evidence pack for each question:
- Accept rate by methodology
- Outcome coverage (outcome_required accept rate)
- Scenario score (weighted outcomes + artifacts)
- Safety/operational rejects (from report.json)
- Qualitative notes (what events actually said)

---

## Additional Questions to Add (Benchmark-Driven)

Data quality:
- Are outcomes being logged consistently per task_id?
- How much of the log is usable (valid JSONL %)?
- What is the false positive rate for trigger matching?

Generalization:
- Do insights remain valid across multiple scenarios in the same domain?
- Do any insights transfer between domains (cross-chip reuse)?
- How fragile are insights to small changes in wording?

Calibration and trust:
- Does confidence correlate with outcomes?
- Are high-confidence insights ever contradicted?
- What is the minimum evidence needed for promotion?

Cost and performance:
- What is the cost per useful insight (events processed / useful insights)?
- What is the time-to-useful-insight per scenario?
- How does method choice affect runtime and storage?

Safety and alignment:
- What % of insights are rejected by safety filters?
- Are safety rejects concentrated in specific chips?
- Do any chips produce harmful content even when blocked?

User impact:
- What proportion of insights are actioned (referenced in artifacts)?
- Which insights change the plan or decision?
- Do insights reduce rework or confusion?
