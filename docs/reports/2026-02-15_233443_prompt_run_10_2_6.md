...
- `~/.spark/advisory_engine.jsonl` retention can truncate older trace rows during longer runs. The harness now reports `engine.matched_trace_rows` vs `engine.expected_trace_rows`; keep `--rounds <= 250` if you need reliable advisory-engine event/latency stats.
- Example (500 rounds, truncated engine window): `docs/reports/2026-02-16_032538_harness_bench_repo_vary_500_v2.json` shows `matched_trace_rows=250` / `expected_trace_rows=500`.
