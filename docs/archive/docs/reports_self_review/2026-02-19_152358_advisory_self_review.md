# Advisory Self-Review (2026-02-19T15:23:58.210866+00:00)
...
## Core Metrics
- Advisory rows: 262
- Advisory trace coverage: 230/262 (87.79%)
- Advice items emitted: 271
- Non-benchmark advisory rows: 262 (excluded 0)
- Engine events: 500
...
- `spark-auto-s7-read-1771514611279` | tool `Read` | source `advisor` | Note-level guidance.
- `spark-auto-s6-edit-1771514611183` | tool `Edit` | source `advisor` | Warning-level guidance.
- `trace-auto-1` | tool `Read` | source `advisor` | Trace-linked live guidance.
- `spark-auto-s2d-read-1771514610666` | tool `Read` | source `cognitive` | Fallback from emitted advice text.
- `spark-auto-s2b-read-1771514610470` | tool `Read` | source `advisor` | Live guidance with policy.
- `spark-auto-s2-read-1771514610276` | tool `Read` | source `advisor` | Live guidance.
- `spark-auto-s1-edit-1771514610117` | tool `Edit` | source `packet` | Use packet guidance.
- `adaf3f8ac1c310de` | tool `Bash` | source `semantic-agentic` | User prefers: the spark here: c:\Users\USER\Desktop\vibeship-spark-pulse
...
- Yes. Top repeated advisories account for ~26.58% of all advice items in this window.

## Top Repeated Advice (Noise Candidates)
- 12x (4.43%) [Caution] Reading files before editing consistently prevents data loss and reduces failed edits by 40% in observed sessions
- 12x (4.43%) Use packet guidance.
- 12x (4.43%) Live guidance.
- 12x (4.43%) Live guidance with policy.
- 12x (4.43%) Fallback from emitted advice text.
- 12x (4.43%) Trace-linked live guidance.
