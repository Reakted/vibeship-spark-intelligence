# Advisory Self-Review (2026-02-19T15:23:21.264648+00:00)
...
## Core Metrics
- Advisory rows: 254
- Advisory trace coverage: 222/254 (87.4%)
- Advice items emitted: 263
- Non-benchmark advisory rows: 254 (excluded 0)
- Engine events: 500
- Engine trace coverage: 500/500 (100.0%)
- Fallback share (delivered): 0.0%
- Strict action rate: 0.2113
...
- `spark-auto-s7-read-1771514598455` | tool `Read` | source `advisor` | Note-level guidance.
- `spark-auto-s6-edit-1771514598361` | tool `Edit` | source `advisor` | Warning-level guidance.
- `trace-auto-1` | tool `Read` | source `advisor` | Trace-linked live guidance.
- `spark-auto-s2d-read-1771514597979` | tool `Read` | source `cognitive` | Fallback from emitted advice text.
- `spark-auto-s2b-read-1771514597877` | tool `Read` | source `advisor` | Live guidance with policy.
- `spark-auto-s2-read-1771514597770` | tool `Read` | source `advisor` | Live guidance.
- `spark-auto-s1-edit-1771514597606` | tool `Edit` | source `packet` | Use packet guidance.
- `080c1179c00897d1` | tool `Bash` | source `trigger` | Never log secrets or tokens; redact sensitive data in logs.
...
- Yes. Top repeated advisories account for ~25.46% of all advice items in this window.

## Top Repeated Advice (Noise Candidates)
- 12x (4.56%) [Caution] Reading files before editing consistently prevents data loss and reduces failed edits by 40% in observed sessions
- 11x (4.18%) Use Bash conservatively with fast validation and explicit rollback safety.
- 11x (4.18%) Use packet guidance.
- 11x (4.18%) Live guidance.
- 11x (4.18%) Live guidance with policy.
- 11x (4.18%) Fallback from emitted advice text.
