# Advisory Self-Review (2026-02-19T15:20:13.452457+00:00)
...
## Core Metrics
- Advisory rows: 196
- Advisory trace coverage: 165/196 (84.18%)
- Advice items emitted: 205
- Non-benchmark advisory rows: 196 (excluded 0)
- Engine events: 500
- Engine trace coverage: 500/500 (100.0%)
- Fallback share (delivered): 0.0%
- Strict action rate: 0.2308
...
- `spark-auto-s7-read-1771514373339` | tool `Read` | source `advisor` | Note-level guidance.
- `spark-auto-s6-edit-1771514373225` | tool `Edit` | source `advisor` | Warning-level guidance.
- `trace-auto-1` | tool `Read` | source `advisor` | Trace-linked live guidance.
- `spark-auto-s2d-read-1771514372692` | tool `Read` | source `cognitive` | Fallback from emitted advice text.
- `spark-auto-s2b-read-1771514372500` | tool `Read` | source `advisor` | Live guidance with policy.
- `spark-auto-s2-read-1771514372305` | tool `Read` | source `advisor` | Live guidance.
- `spark-auto-s1-edit-1771514372152` | tool `Edit` | source `packet` | Use packet guidance.
...
- Yes. Top repeated advisories account for ~25.35% of all advice items in this window.

## Top Repeated Advice (Noise Candidates)
- 12x (5.85%) [Caution] Reading files before editing consistently prevents data loss and reduces failed edits by 40% in observed sessions
- 10x (4.88%) Use Bash conservatively with fast validation and explicit rollback safety.
- 9x (4.39%) Reasoning: another user is running a heavy analytics query (weight=50)
- 7x (3.41%) I'm talking about how they use their login system and how they make agents be utilised through md files of agents joining through the curl system. They had also
- 7x (3.41%) Constraint: observable** (every step changes something)
- 7x (3.41%) Macro (often works): Bash→Read→Grep. Use this sequence when appropriate to reduce thrash.
