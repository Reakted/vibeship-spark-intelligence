# Advisory Self-Review (2026-02-19T15:22:46.856407+00:00)
...
## Core Metrics
- Advisory rows: 238
- Advisory trace coverage: 206/238 (86.55%)
- Advice items emitted: 247
- Non-benchmark advisory rows: 238 (excluded 0)
- Engine events: 500
- Engine trace coverage: 500/500 (100.0%)
- Fallback share (delivered): 0.0%
- Strict action rate: 0.2206
...
- `1665db8294c29eb3` | tool `Read` | source `trigger` | Validate authentication inputs server-side and avoid trusting client checks.
- `d45a3a15bfc0d9db` | tool `Read` | source `trigger` | Validate authentication inputs server-side and avoid trusting client checks.
- `c297a75472ac3597` | tool `Read` | source `trigger` | Validate authentication inputs server-side and avoid trusting client checks.
- `d8a042baac85fcde` | tool `Read` | source `trigger` | Validate authentication inputs server-side and avoid trusting client checks.
- `spark-auto-s7-read-1771514533211` | tool `Read` | source `advisor` | Note-level guidance.
- `spark-auto-s6-edit-1771514533100` | tool `Edit` | source `advisor` | Warning-level guidance.
- `trace-auto-1` | tool `Read` | source `advisor` | Trace-linked live guidance.
- `spark-auto-s2d-read-1771514532570` | tool `Read` | source `cognitive` | Fallback from emitted advice text.
...
- Yes. Top repeated advisories account for ~23.87% of all advice items in this window.

## Top Repeated Advice (Noise Candidates)
- 12x (4.86%) [Caution] Reading files before editing consistently prevents data loss and reduces failed edits by 40% in observed sessions
- 11x (4.45%) Use Bash conservatively with fast validation and explicit rollback safety.
- 9x (3.64%) Reasoning: another user is running a heavy analytics query (weight=50)
- 9x (3.64%) Validate authentication inputs server-side and avoid trusting client checks.
- 9x (3.64%) Use packet guidance.
- 9x (3.64%) Live guidance.
...
## Bad Outcome Records
- trace `ead96dd3237ae0a9` | source `auto_created` | insight `None` | tool:Glob
