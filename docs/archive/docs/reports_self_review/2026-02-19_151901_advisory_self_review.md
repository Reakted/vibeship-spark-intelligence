# Advisory Self-Review (2026-02-19T15:19:01.917788+00:00)
...
## Core Metrics
- Advisory rows: 180
- Advisory trace coverage: 150/180 (83.33%)
- Advice items emitted: 189
- Non-benchmark advisory rows: 180 (excluded 0)
- Engine events: 500
- Engine trace coverage: 500/500 (100.0%)
- Fallback share (delivered): 0.0%
- Strict action rate: 0.2222
- Strict effectiveness rate: 0.6429
- Trace mismatch count: 46
...
- `spark-auto-s7-read-1771514287310` | tool `Read` | source `advisor` | Note-level guidance.
- `spark-auto-s6-edit-1771514287198` | tool `Edit` | source `advisor` | Warning-level guidance.
- `trace-auto-1` | tool `Read` | source `advisor` | Trace-linked live guidance.
- `spark-auto-s2d-read-1771514286723` | tool `Read` | source `cognitive` | Fallback from emitted advice text.
- `spark-auto-s2b-read-1771514286628` | tool `Read` | source `advisor` | Live guidance with policy.
- `spark-auto-s2-read-1771514286485` | tool `Read` | source `advisor` | Live guidance.
- `spark-auto-s1-edit-1771514286334` | tool `Edit` | source `packet` | Use packet guidance.
- `73f5522fa9b59b19` | tool `WebSearch` | source `semantic-agentic` | is it possible to now build something where this podcast can have the avatar of Spark reading talking with lip sync within a video, so that we can improve podca
...
- Yes. Top repeated advisories account for ~26.97% of all advice items in this window.

## Top Repeated Advice (Noise Candidates)
- 12x (6.35%) [Caution] Reading files before editing consistently prevents data loss and reduces failed edits by 40% in observed sessions
- 10x (5.29%) Use Bash conservatively with fast validation and explicit rollback safety.
- 9x (4.76%) Reasoning: another user is running a heavy analytics query (weight=50)
- 7x (3.7%) I'm talking about how they use their login system and how they make agents be utilised through md files of agents joining through the curl system. They had also
- 7x (3.7%) Constraint: observable** (every step changes something)
- 6x (3.17%) Use Read conservatively with fast validation and explicit rollback safety.
