# Advisory Self-Review (2026-02-19T15:23:08.551691+00:00)
...
## Core Metrics
- Advisory rows: 246
- Advisory trace coverage: 214/246 (86.99%)
- Advice items emitted: 255
- Non-benchmark advisory rows: 246 (excluded 0)
- Engine events: 500
- Engine trace coverage: 500/500 (100.0%)
- Fallback share (delivered): 0.0%
- Strict action rate: 0.2
...
- `de4cecba911e20b4` | tool `Bash` | source `prefetch` | Use Bash on critical-path tasks first; unblock dependencies before parallel work.
- `spark-auto-s7-read-1771514578874` | tool `Read` | source `advisor` | Note-level guidance.
- `spark-auto-s6-edit-1771514578765` | tool `Edit` | source `advisor` | Warning-level guidance.
- `trace-auto-1` | tool `Read` | source `advisor` | Trace-linked live guidance.
- `spark-auto-s2d-read-1771514578245` | tool `Read` | source `cognitive` | Fallback from emitted advice text.
- `spark-auto-s2b-read-1771514578053` | tool `Read` | source `advisor` | Live guidance with policy.
- `spark-auto-s2-read-1771514577943` | tool `Read` | source `advisor` | Live guidance.
- `spark-auto-s1-edit-1771514577796` | tool `Edit` | source `packet` | Use packet guidance.
...
- Yes. Top repeated advisories account for ~24.7% of all advice items in this window.

## Top Repeated Advice (Noise Candidates)
- 12x (4.71%) [Caution] Reading files before editing consistently prevents data loss and reduces failed edits by 40% in observed sessions
- 11x (4.31%) Use Bash conservatively with fast validation and explicit rollback safety.
- 10x (3.92%) Use packet guidance.
- 10x (3.92%) Live guidance.
- 10x (3.92%) Live guidance with policy.
- 10x (3.92%) Fallback from emitted advice text.
