# Advisory Self-Review (2026-02-19T15:17:59.134829+00:00)
...
## Core Metrics
- Advisory rows: 171
- Advisory trace coverage: 141/171 (82.46%)
- Advice items emitted: 180
- Non-benchmark advisory rows: 171 (excluded 0)
- Engine events: 500
- Engine trace coverage: 500/500 (100.0%)
- Fallback share (delivered): 0.0%
- Strict action rate: 0.2295
- Strict effectiveness rate: 0.6429
...
- `ea90fded3868d38b` | tool `WebSearch` | source `semantic-agentic` | is it possible to now build something where this podcast can have the avatar of Spark reading talking with lip sync within a video, so that we can improve podca
- `1b5c5d84ac7f12be` | tool `WebSearch` | source `semantic-agentic` | is it possible to now build something where this podcast can have the avatar of Spark reading talking with lip sync within a video, so that we can improve podca
- `spark-auto-s7-read-1771514219555` | tool `Read` | source `advisor` | Note-level guidance.
- `spark-auto-s6-edit-1771514219479` | tool `Edit` | source `advisor` | Warning-level guidance.
- `trace-auto-1` | tool `Read` | source `advisor` | Trace-linked live guidance.
- `spark-auto-s2d-read-1771514219140` | tool `Read` | source `cognitive` | Fallback from emitted advice text.
- `spark-auto-s2b-read-1771514219041` | tool `Read` | source `advisor` | Live guidance with policy.
- `spark-auto-s2-read-1771514218866` | tool `Read` | source `advisor` | Live guidance.
...
- Yes. Top repeated advisories account for ~28.34% of all advice items in this window.

## Top Repeated Advice (Noise Candidates)
- 12x (6.67%) [Caution] Reading files before editing consistently prevents data loss and reduces failed edits by 40% in observed sessions
- 10x (5.56%) Use Bash conservatively with fast validation and explicit rollback safety.
- 9x (5.0%) Reasoning: another user is running a heavy analytics query (weight=50)
- 7x (3.89%) I'm talking about how they use their login system and how they make agents be utilised through md files of agents joining through the curl system. They had also
- 7x (3.89%) Constraint: observable** (every step changes something)
- 6x (3.33%) Use Read conservatively with fast validation and explicit rollback safety.
