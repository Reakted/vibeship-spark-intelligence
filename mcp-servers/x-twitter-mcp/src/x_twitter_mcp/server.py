...
def conditional_tool(name: str, description: str):
    """Decorator to conditionally register tools based on permissions."""
    def decorator(func: Callable) -> Callable:
        if is_tool_enabled(name):
            return server.tool(name=name, description=description)(func)
        else:
            # Return the function unchanged but not registered
            return func
    return decorator
...
async def get_tweet_details(tweet_id: str) -> Dict:
    ...
    result = tweet.data.data if tweet.data else None
...
async def create_list(name: str, description: Optional[str] = None, private: bool = False) -> Dict:
    """Creates a new list."""
    if not check_rate_limit("list_actions"):
        raise Exception("List action rate limit exceeded")
    client, _ = initialize_twitter_clients()
...
async def send_dm(participant_id: str, text: str) -> Dict:
    ...
    if not check_rate_limit("dm_actions"):
        raise Exception("DM action rate limit exceeded")
    client, _ = initialize_twitter_clients()
